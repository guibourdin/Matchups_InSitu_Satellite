...
% for i = 1:NOCfiles
  ...
  %                         p = m_pcolor(sellonOC, latOCini, latOCini./latOCini-1); alpha(p,0.5); hold on
              m_line([sellonOC(1,:)'; sellonOC(end,:)'; sellonOC(:,1); sellonOC(:,end)],...
                [latOCini(1,:)'; latOCini(end,:)'; latOCini(:,1); latOCini(:,end)], 'linewi',2,'color','k');     % Area outline ...
  %                         m_plot(im_lim(:,2),im_lim(:,1),'Color', 'r');
              ...
              temp{1,ilon}(temp{1,ilon}<0) = temp{1,ilon}(temp{1,ilon}<0)+360;
  ...
  %                             max(temp{1,id_plotinsitu})]);
    ...
    if ~isfolder('Corrupted_files')
      mkdir('Corrupted_files')
    end
    movefile(ncOCfiles(i).name, fullfile('Corrupted_files', ncOCfiles(i).name))
    fprintf(' - corrupted, moved to corrupted folder\n')
  end
  latOCini = [];
  lonOCini =[];
  sellonOC = [];
